import Image from "next/image";

export default function Navbar() {
    return (
        <header className="w-full">

            {/* Top Bar */}
            <div className="flex h-9 items-center justify-center gap-2 bg-black px-4 text-[11px] font-medium tracking-[0.08em] text-white sm:h-10 sm:text-xs">
        <span className="text-center">
          Transport GRATUIT la comenzi peste 299 RON (vezi condiții)
        </span>
                <span aria-hidden="true">→</span>
            </div>

            {/* Main Navbar */}
            <div className="grid h-[76px] grid-cols-3 items-center bg-[#f3f3f3] px-4 sm:px-6">

                {/* Left - Menu */}
                <div className="flex items-center justify-start w-[120px]">
                    <button
                        type="button"
                        aria-label="Open menu"
                        className="flex h-10 w-10 items-center justify-center"
                    >
                        <Image
                            src="/icons/menuButton.svg"
                            alt=""
                            width={26}
                            height={26}
                            className="h-7 w-7"
                        />
                    </button>
                </div>

                {/* Center - Logo (clickable → go home later) */}
                <div className="flex items-center justify-center">
                    <button
                        type="button"
                        aria-label="Go to homepage"
                        className="flex items-center justify-center"
                    >
                        <Image
                            src="/logos/beastLockerLogo.svg"
                            alt=""
                            width={170}
                            height={40}
                            className="h-auto w-[150px] sm:w-[170px]"
                            priority
                        />
                    </button>
                </div>

                {/* Right - Icons */}
                <div className="flex items-center justify-end gap-4 sm:gap-5 pl-10">

                    <button
                        type="button"
                        aria-label="Search"
                        className="flex h-10 w-10 items-center justify-center"
                    >
                        <Image
                            src="/icons/searchIcon.svg"
                            alt=""
                            width={22}
                            height={22}
                            className="h-[22px] w-[22px]"
                        />
                    </button>

                    <button
                        type="button"
                        aria-label="Account"
                        className="flex h-10 w-10 items-center justify-center"
                    >
                        <Image
                            src="/icons/accountIcon.svg"
                            alt=""
                            width={22}
                            height={22}
                            className="h-[22px] w-[22px]"
                        />
                    </button>

                    <button
                        type="button"
                        aria-label="Cart"
                        className="flex h-10 w-10 items-center justify-center"
                    >
                        <Image
                            src="/icons/cartIcon.svg"
                            alt=""
                            width={22}
                            height={22}
                            className="h-[22px] w-[22px]"
                        />
                    </button>

                </div>

            </div>
        </header>
    );
}